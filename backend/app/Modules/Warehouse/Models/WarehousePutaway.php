...
class WarehousePutaway extends Model
{
    ...
    {
        return $this->hasMany(WarehousePutawayItem::class, 'warehouse_putaway_id');
    }
    ...
    {
        return $query->where('putaway_type', $type);
    }
    ...
    {
        $totalToPutaway = $this->items->sum('quantity_to_putaway');
        $totalPutaway = $this->items->sum('quantity_putaway');
        
        if ($totalToPutaway <= 0) {
            return 0;
        }

        return round(($totalPutaway / $totalToPutaway) * 100, 2);
    }
}
